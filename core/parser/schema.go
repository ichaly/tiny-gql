...
func ParseSchema(source *lexer.Input) (*ast.SchemaDocument, error) {
	p := parser{
		lexer: lexer.NewLexer(source),
	}
	doc, err := p.parseSchemaDocument(), p.err
	if err != nil {
		return nil, err
	}
	return doc, nil
...
func (p *parser) parseSchemaDocument() *ast.SchemaDocument {
	...
	for p.peek().Kind != lexer.EOF {
		...
		if p.peek().Kind == lexer.Block || p.peek().Kind == lexer.String {
...
func (p *parser) parseDescription() string {
	...
	if token.Kind != lexer.Block && token.Kind != lexer.String {
		return ""
	}
